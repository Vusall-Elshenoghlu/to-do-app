...
import { useResetPassword } from './actions/reset-password.mutation';
...
const ResetPassword = () => {
    ...
    const initialValues: IResetPasswordFormValues = { password: '', confirmPassword: '' };

    const rules: { [key: string]: FormRule[] } = useMemo(
        () => ({
            password: [
                { required: true, message: translate('input_required') },
                { min: 8, message: translate('password_min_length') },
            ],
            confirmPassword: [
                { required: true, message: translate('input_required') },
                ({ getFieldValue }) => ({
    ...
    const cardVariants = { hidden: { opacity: 0, y: 30, scale: 0.95 }, visible: { opacity: 1, y: 0, scale: 1, transition: { type: 'spring', stiffness: 100, damping: 15, duration: 0.6 } } };
    const itemVariants = { hidden: { opacity: 0, x: -20 }, visible: { opacity: 1, x: 0, transition: { type: 'spring', stiffness: 100, damping: 12 } } };
            ...
            <motion.div className={classes.panel} variants={cardVariants}>
                <Card className={classes.card}>
                    <motion.div variants={itemVariants}>
                        <div className={classes.logo}><ResetPasswordIcon /></div>
                    ...
                    <Form initialValues={initialValues} onFinish={onSubmit} layout="vertical" autoComplete="off">
                        <motion.div variants={itemVariants}>
                        ...
                        <motion.div variants={itemVariants}>
                            <Form.Item rules={rules.confirmPassword} name="confirmPassword" label="Confirm Password" className={classes.formItem}>
                    ...
                    <motion.div variants={itemVariants}>
                        <div className={classes.footer}>
                            Remember your password? <a href={Routes.login}>Sign In</a>
                        </div>
                    </motion.div>
                </Card>
            </motion.div>
